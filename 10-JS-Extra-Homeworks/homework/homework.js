...
for ( var i= 0; i< string.length ; i++){
  if (!acumulador.hasOwnProperty(string[i])){
    acumulador[string[i]] = 0
  }
  acumulador[string[i]] ++ // +=1
}
return acumulador
}
...
// var frase = str.split(" ").map(function (x){
...
function deleteAbc(cadena){
  ...
  for ( var i = 0 ; i< cadena.length ; i++){
    if (cadena[i] !== "a" && cadena[i] !== "b" && cadena[i] !== ("c")){ // === "a" || cadena[i]=== "b"
